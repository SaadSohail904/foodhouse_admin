...
export default class ChangePassword extends Component {
    ...
    changepswd=()=>{
        if(this.state.newpswd.trim()===''){
        ...
        else if(this.state.newpswd.length<4 || this.state.confirmpswd.length<4){
            this.setState({alertshow:true,alertshowsuccess:false,alerttext:'Password length must be at least 4 characters'});
        }
            ...
            .then(res => {
                ...
                if(res.data.statusCode===200){
                    this.setState({newpswd:'',confirmpswd:''});
